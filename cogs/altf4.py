...
class Altf4(commands.Cog):
    ...
    async def AltF4(self, ctx):
        ...
        embed.add_field(
            name="Board of Directors", value="Elias Sicario\n Ava Arienti", inline=False
        )
        embed.add_field(
            name="Chief Executive Officer", value="Jordy Jetson", inline=False
        )
        embed.add_field(
            name="Chief Financial Officer", value="Window Payne", inline=False
        )
    ...
    async def leaderboard(ctx, limit=10):
        ...
        for user in db.keys():
            ...
            em = discord.Embed(
                title=f"Top {limit} Earners!",
                description="This is decided on the basis of Total Lifetime Ticket Value",
                color=discord.Color(0xFA43EE),
            )
            keys = keys[:limit]
            for index, amt in enumerate(keys):
                em.add_field(
                    name=f"{index + 1}. {leader_board[amt]}",
                    value=f"${amt:,}",
                    inline=False,
                )
    ...
    async def weeklyhours(ctx, limit=100):
        ...
        for user in db.keys():
            ...
            em = discord.Embed(
                title=f"Weekly hours for employees!",
                description="Make sure to log these in excel before purging!!!",
                color=discord.Color(0xFA43EE),
            )
            keys = keys[:limit]
            for index, amt in enumerate(keys):
                em.add_field(
                    name=f"{index + 1}. {weekly_hours[amt]}",
                    value=f"{amt}",
                    inline=False,
                )
    ...
    async def flight(ctx, destination=None, passengers=None):
        ...
        fm = new_embed(
            title=f"Ticket Collected {ctx.author.name}",
    ...
    async def shstats(ctx, user: discord.User = None):
        ...
        if user["clocked_in"] == False:
            ...
            start_time = datetime.datetime.strptime(
                user["clock_start"], "%a, %b %d, %Y\n\n%I:%M:%S %p"
            )
            shift_time_str = calculate_shift_time(start_time)

            try:
                em = new_embed(
                    title=f"{userat}'s Shift Stats",
                    fields={
    ...
    async def clock(ctx, username: discord.User = None):
        ...
        if user["clocked_in"] is not None and user["clocked_in"] == True:
            ...
            fm = new_embed(
                title=f"{uid.name} has clocked out",
                fields={
            ...
            fm = new_embed(
                title=f"{uid.name} has clocked in",
    ...
    def calculate_shift_time(shift_time=None):
        ...
        shift_time_str = "{} Hours, {} Minutes, {} Seconds".format(
            shift_time_hrs, shift_time_mins, shift_time_secs
        )
    ...
    async def r(ctx, user: discord.User = None):
        ...
        if earnings > 0:  # generate invoice, and fork to log channel
            earning = earnings * -1
            fm = new_embed(
                title=f"{user.name}'s Invoice Created",
                fields={
                    "Payment Recieved": "${:,}".format(earnings),
                    "Remaining Balance": "${:,}".format(user["balance_owed"] + earning),
                },
            )
            await ctx.send(embed=fm)
            ...
            earning = earnings * -1
            fm = new_embed(
                title=f"{user.name}'s remove log",
